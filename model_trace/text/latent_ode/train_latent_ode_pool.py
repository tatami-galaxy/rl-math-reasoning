"""
Train latent ODE with cumulative-mean-pooled trajectory points.

Each trajectory point is [pool_t ; e_t] where:
  - e_t is the independent Qwen embedding of segment t
  - pool_t = mean(e_0, e_1, ..., e_t)  (cumulative mean of all segments so far)

This gives the ODE a stable "summary of reasoning so far" alongside the
current segment — parameter-free unlike EMA.

Usage:
CUDA_VISIBLE_DEVICES=0 python -m model_trace.text.latent_ode.train_latent_ode_pool \\
...
class Config:
    ...
    output_dir: str = field(default="models/latent_ode_pool")
...
def build_pool_trajectories(
    embeddings: list[np.ndarray],
) -> list[np.ndarray]:
    """Convert independent segment embeddings into [pool_t ; e_t] trajectory points.

    For each trace with embeddings (T, D), produces (T, 2D) where:
      - first D dims = cumulative mean: pool_t = mean(e_0, ..., e_t)
      - last  D dims = raw segment embedding e_t
    """
    result = []
    for emb in embeddings:  # emb: (T, D)
        T = emb.shape[0]
        cumsum = np.cumsum(emb, axis=0)                       # (T, D)
        counts = np.arange(1, T + 1, dtype=emb.dtype)[:, None]  # (T, 1)
        pool = cumsum / counts                                 # (T, D)
        combined = np.concatenate([pool, emb], axis=1)         # (T, 2D)
...
class TraceDataset(Dataset):
    """Each item is a (T_i, 2D) float32 tensor — one pool-augmented trajectory."""
...
def build_dataset(config: Config, embedder: QwenEmbedder, max_traces: int | None = None) -> TraceDataset:
    ...
    # Build cumulative-mean-pooled trajectories: (T_i, D) -> (T_i, 2D)
    print("Building cumulative-mean-pooled trajectories...")
    pool_traces = build_pool_trajectories(per_trace)

    return TraceDataset(pool_traces, segments=all_segments_lists)
...
def main():
    ...
    # Model — d_embed is 2x raw Qwen dim (pool + current)
